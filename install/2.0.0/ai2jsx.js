// ai2jsx is a script for Adobe Illustrator that converts your Illustrator document into html and css.
...
// =====================================
// How to install ai2jsx
// =====================================

// - Move the ai2jsx.js file into the Illustrator folder where scripts are located.
// - For example, on Mac OS X running Adobe Illustrator CC 2014, the path would be: // Adobe Illustrator CC 2014/Presets/en_US/Scripts/ai2jsx.jsx
...
// - Run the script by choosing: File > Scripts > ai2jsx
// - Go to the folder containing your Illustrator file. Inside will be a folder called ai2jsx-output.
...
// http://wwwimages.adobe.com/content/dam/Adobe/en/devnet/pdf/illustrator/scripting/Readme.txt
  ...
  // ================================================
  // ai2jsx and config settings
  // ================================================

  // These are base settings that are overridden by text block settings in
  // the .ai document and settings containing in ai2jsx-config.json files
  var defaultSettings = {
    ...
    'max_width': '',
    'output': 'multiple-files', // Options: one-file, multiple-files
    'project_name': '', // Defaults to the name of the AI file
    'project_type': '',
    'html_output_path': '/ai2jsx-output/',
    'html_output_extension': '.jsx',
    'image_output_path': '../../public/media/',
    ...
    'inline_svg': true, // Embed background image SVG in HTML instead of loading a file
    ...
    // List of settings to include in the "ai2jsx-settings" text block
    'settings_block': [
      'settings_version',
      'image_format',
      'responsiveness',
  ...
  try {
    if (!isTestedIllustratorVersion(app.version)) {
      warn('Ai2jsx has not been tested on this version of Illustrator.');
    }
    ...
    if (app.activeDocument.documentColorSpace != DocumentColorSpace.RGB) {
      error('You should change the document color mode to "RGB" before running ai2jsx (File>Document Color Mode>RGB Color).');
    }
    if (app.activeDocument.activeLayer.name == 'Isolation Mode') {
      error('Ai2jsx is unable to run because the document is in Isolation Mode.');
    }
    ...
    // TODO: find a better way to detect this condition (mask can be renamed)
      error('Ai2jsx is unable to run because you are editing an Opacity Mask.');
    }
  ...
  // warn about duplicate artboard names
    ...
    // ================================================
    progressBar = new ProgressBar({ name: 'Ai2jsx progress', steps: calcProgressBarSteps() });
    ...
    forEachUsableArtboard(function(activeArtboard, abIndex) {
      ...
      artboardContent.html += generateArtboardDiv(activeArtboard, breakpoints, settings) +
       imageData.html +
       textData.html +
       '\t\t\t</div>\r';
      artboardContent.html = artboardContent.html.replace(/\t/g, '  ');
      artboardContent.css += generateArtboardCss(activeArtboard, textData.styles, settings);
      artboardContent.jsx = generateArtboardJsx(activeArtboard, artboardContent.html, settings);
      artboardContent.width = getArtboardWidth(activeArtboard);
    ...
    generateWrapperFile(fileContentArr, getDocumentName(), settings);
    ...
    // Write configuration file with graphic metadata
      ...
      var yamlStr = generateYamlFileContent(breakpoints, settings);
  ...
  // =====================================
  // ai2jsx specific utility functions
  // =====================================
  ...
  function convertCssProps(prop) {
    return prop.replace(/-(.)/, function(match, firstLetter) {
      return firstLetter.toUpperCase();
    });
  }

  // ==================================
  // ai2jsx program state and settings
  // ==================================
  ...
  function detectScriptEnvironment() {
    ...
    // Handle case where user seems to be at NYT but is running ai2jsx outside of Preview
    if (env == 'nyt' && !fileExists(docPath + '../config.yml')) {
      if (confirm('You seem to be running ai2jsx outside of NYT Preview.\nContinue in non-Preview mode?', true)) {
        env = ''; // switch to non-nyt context
      } else {
        error('Ai2jsx should be run inside a Preview project.');
      }
    }
    return env;
  }
  ...
  function initSpecialTextBlocks() {
    var rxp = /^ai2jsx-(css|js|html|settings|text)\s*$/;
    var settings = null;
    var code = {};
    ...
    // var contents = thisFrame.contents; // caused MRAP error in AI 2017
      ...
      // Reset the name of any non-settings text boxes with name ai2jsx-settings
      if (type != 'settings' && thisFrame.name == 'ai2jsx-settings') {
        thisFrame.name = '';
      }
      if (type == 'settings' || type == 'text') {
        ...
        // set name of settings block, so it can be found later using getByName()
          thisFrame.name = 'ai2jsx-settings';
        }
  ...
  // Derive ai2jsx program settings by merging default settings and overrides.
  ...
  function importNewYorkTimesSettings(settings) {
    ...
    settings.project_type = yamlConfig.project_type == 'ai2jsx' ? 'ai2jsx' : '';
    if (!folderExists(docPath + '../public/') ||
      (settings.project_type != 'ai2jsx' && !folderExists(docPath + '../src/'))) {
    ...
    if (settings.project_type == 'ai2jsx') {
  ...
  // Looks for settings file in the ai2jsx script directory and/or the .ai document directory
  function readExternalSettings() {
    var settingsFile = 'ai2jsx-config.json';
  ...
  // with settings to override the default ai2jsx settings.
  ...
  function createSettingsBlock(settings) {
    ...
    var top = bounds[1];
    var settingsLines = ['ai2jsx-settings'];
    ...
    try {
      layer = doc.layers.getByName('ai2jsx-settings');
      layer.locked = false;
    } catch (e) {
      layer = doc.layers.add();
      layer.zOrder(ZOrderMethod.BRINGTOFRONT);
      layer.name = 'ai2jsx-settings';
    }
  ...
  function updateSettingsEntry(key, value) {
    var block = doc.textFrames.getByName('ai2jsx-settings');
    var entry = key + ': ' + value;
    var updated = false;
    var lines;
    if (!block) return;
  ...
  // Add ai2jsx settings from a text block to a settings object
  ...
  function showCompletionAlert(showPrompt) {
    ...
    if (showPrompt) {
      ...
      alertText += rule + 'ai2jsx-nyt5 v' + scriptVersion;
  ...
  // ======================================
  // ai2jsx AI document reading functions
  // ======================================
  ...
  // capture portion of name after colon
    ...
    var widthStr = (/^ai2jsx-(\d+)/.exec(name) || [])[1];
    if (widthStr) {
  ...
  function generateParagraphHtml(pData, baseStyle, pStyles, cStyles) {
    ...
    if (diff) {
      html = '<p className=\'' + getTextStyleClass(diff, pStyles, 'pstyle') + '\'>';
    } else {
      html = '<p>';
    }
  ...
  function getTextFrameCss(thisFrame, abBox, pgData) {
    ...
    return 'className=\'' + classes + '\' style={' + convertStyleString(styles) + '}';
  }
  ...
  function getBasicSymbolCss(geom, style, abBox, opts) {
    ...
    return 'style={' + convertStyleString(styles.join('; ')) + '}';
  }
  ...
  function exportSymbolAsHtml(item, geometries, abBox, opts) {
    ...
    for (var i = 0; i < geometries.length; i++) {
      ...
      geom.center = [x, y];
      html += '\r\t\t\t' + '<div className=\'' + getSymbolClass() + '\' ' + properties +
  ...
  function exportSymbols(lyr, ab, masks, opts) {
    ...
    if (html) {
      html = '\t\t<div className=\'' + nameSpace + 'symbol-layer ' + nameSpace + getLayerName(lyr) + '\'>' + html + '\r\t\t</div>\r';
    }
    return {
      html: html,
      items: items,
  ...
  // setting: value from ai2jsx settings (e.g. "auto" "png")
  ...
  function exportImage(imgName, format, ab, masks, layer, settings) {
    ...
    const abName = getArtboardName(ab);
    var outputDir = pathJoin(getImageFolder(settings), abName);
    checkForOutputFolder(outputDir, abName);

    var outputPath = pathJoin(outputDir, imgFile);
  ...
  function generateInlineSvg(imgPath, imgClass, imgStyle, settings) {
    ...
    var attr = ' className=\'' + imgClass + '\'';
    if (imgStyle) {
      attr += ' style={' + convertStyleString(imgStyle) + '}';
    }
    svg = svg.replace(/<\?xml.*?\?>/, '');
  ...
  function generateImageHtml(imgFile, imgId, imgClass, imgStyle, ab, settings) {
    ...
    var html; var src;
    if (imgDir === null) {
      var imageOutputPathParts = settings.image_output_path.split('public');
      imgDir = imageOutputPathParts.length > 1 ? imageOutputPathParts[1] : imageOutputPathParts[0];
    }
    src = pathJoin(imgDir, getArtboardName(ab), imgFile);

    html = '\t\t\t\t<img id=\'' + imgId + '\' className=\'' + imgClass + '\' alt=\'' + imgAlt + '\'';

    if (imgStyle) {
      html += ' style={' + convertStyleString(imgStyle) + '}';
    }

    html += ' src={getStatic(\'/' + src + '\')}';
    html += '/>\r';
    return html;
  }
  ...
  function createPromoImage(settings) {
    ...
    var format = getPromoImageFormat(ab, settings);
    var imgFile = getImageFileName(getDocumentName() + '-promo', format);
    var outputPath = docPath + imgFile;
    var opts = {
  ...
  function copyArtboardForImageExport(ab, masks, layers) {
    ...
    var sourceLayers = layers || toArray(doc.layers);
    var destLayer = doc.layers.add();
    var destGroup = doc.groupItems.add();
    var itemCount = 0;
    var groupPos; var group2; var doc2;
  ...
  function reapplyEffectsInSVG(svg) {
    ...
    function replace(a, b) {
      ...
      retn = 'style={' + convertStyleString(style) + '}';
      if (b.indexOf('--') === 0) {
      // restore original id
  ...
  // ===================================
  // ai2jsx output generation functions
  // ===================================
  ...
  function assignArtboardContentToFile(name, abData, outputArr) {
    ...
    if (!obj) {
      obj = { name: name, html: '', js: '', css: '', jsx: '', width: 0 };
      outputArr.push(obj);
    }
  ...
  function generateArtboardDiv(ab, breakpoints, settings) {
    ...
    html += '\t\t\t<div id=\'' + divId + '\' className=\'' + classnames + '\' style={' + convertStyleString(inlineStyle) + '}';
  ...
  function generateArtboardJsx(ab, html, settings) {
    var name = getArtboardName(ab);
    var jsx = '';
    jsx += "import React from 'react';\r";
    jsx += "import getStatic from 'Utils/getStatic';\r\r";
    jsx += "import stylesheet from './styles.module.scss';\r\r";
    jsx += 'const ' + name + ' = () => (\r';
    jsx += '  <div className=\'' + name + ' artboard\'>\r';
    jsx += '    <div id=\'g-' + name + '-box\'>\r';
    jsx += html;
    jsx += '    </div>\r';
    jsx += '    <style jsx>{stylesheet}</style>\r';
    jsx += '  </div>\r';
    jsx += ');\r\r';
  ...
  function generatePageCss(containerId, settings) {
    ...
    css += t2 + 'svg' + '{ vector-effect: non-scaling-stroke; }';

    return css;
  }

  function generatePageScss(containerId, settings, artboardcss) {
    var pageCss = generatePageCss(containerId, settings);

    var scss = '';
    scss += '.artboard {\r';
    scss += pageCss.replace(/\t/g, '  ') + '\r';
    scss += artboardcss.replace(/\t/g, '  ');
    scss += '}';
  ...
  function generateYamlFileContent(breakpoints, settings) {
    var lines = [];
    lines.push('ai2jsx_version: ' + scriptVersion);
    lines.push('project_type: ' + settings.project_type);
    lines.push('tags: ai2jsx');
  ...
  //
    var resizer = function(containerId, opts) {
      ...
      function throttle(func, wait) {
        var timeout = null; var previous = 0;
        function run() {
          previous = Date.now();
          timeout = null;
          func();
  ...
  function outputLocalPreviewPage(textForFile, localPreviewDestination, settings) {
    ...
    settings.ai2jsxPartial = textForFile; // TODO: don't modify global settings this way
  ...
  function generateOutputHtml(content, pageName, settings) {
    ...
    var textForFile, html, js, css, scss, commentBlock;
    ...
    // comments
    commentBlock = '/* Generated by ai2jsx v' + scriptVersion + ' - ' +
    ...
    // SCSS
    var sassForFile = generatePageScss(containerId, settings, content.css);

    // JS
    // js = content.js + responsiveJs;

    // combine text for final file
    textForFile = '\r' + commentBlock + '\r' + html + '\r' + '/* End ai2jsx' + ' - ' + getDateTimeStamp() + ' */\r';

    // remove bad comments and tags
    textForFile = textForFile.replace(/<!--.*-->/g, '');
    textForFile = textForFile.replace(/x\w*:\w*=".*"/g, '');

    htmlFileDestinationFolder = docPath + settings.html_output_path;
    checkForOutputFolder(htmlFileDestinationFolder, 'output_path');

    var componentModuleDestinationFolder = htmlFileDestinationFolder + pageName;
    checkForOutputFolder(componentModuleDestinationFolder, pageName);

    var htmlFileDestination = htmlFileDestinationFolder + pageName + '/index.jsx';
    var scssFileDestination = htmlFileDestinationFolder + pageName + '/styles.module.scss';
    //
    // if (settings.output == 'one-file' && settings.project_type == 'ai2jsx') {
    ...
    saveTextFile(scssFileDestination, sassForFile);
  }

  function generateWrapperFile(artboards, pageName, settings) {
    var imports = '';
    var layouts = '';

    forEach(artboards, function(ab) {
      imports += 'import ' + ab.name + ' from \'./' + ab.name + '\';\r';
      layouts += '  ' + ab.name + ': {\r';
      layouts += '    breakpoint: ' + ab.width + ',\r';
      layouts += '    component: ' + ab.name + ',\r';
      layouts += '  },\r';
    });

    var jsx = '';
    jsx += "import React from 'react';\r";
    jsx += "import useLayoutBreakpoints from 'Utils/hooks/useLayoutBreakpoints';\r\r";
    ...
    jsx += layouts;
    jsx += '};\r\r';
    jsx += 'const ' + pageName + ' = () => {\r';
    jsx += '  const layout = useLayoutBreakpoints(LAYOUTS);\r\r';
    jsx += '  return (\r';
    jsx += '    <div className=\'ai-' + pageName.toLowerCase() + '\'>\r';
    jsx += '      {layout}\r';
    jsx += '    </div>\r';
    jsx += '  );\r';
    jsx += '};\r\r';
    jsx += 'export default ' + pageName + ';';

    var fileDestination = docPath + settings.html_output_path + 'index.jsx';
    saveTextFile(fileDestination, jsx);
  }
